select
    ...
    greatest((case when
        offers.discount_percentage-coalesce(offers.commercial_discount,0)<40
            ...
            else least(2*offers.offered_price,university_offers.full_price) end)*{0},149.9) pre_enrollment_fees
from offers
...
where
    1=1
    and offers.enabled
    and levels.parent_id={6}
    and kinds.parent_id={5}
    and {1} coalesce(offers.billing_integration,false)
    {2}and offers.university_id {4} in ({3})
