select distinct
    ...
    ,coalesce(offers.status,'enabled') status
    ,coalesce(offers.commercial_discount,0.) commercial_old
    ,offers.show_on_main_search
    ,offers.visible
    ,max(offers.discount_percentage) over (partition by courses.id) max_discount
from offers
    ...
    join shifts on shifts.name=courses.shift
        and shifts.parent_id is not null
...
where
    ...
    {6}and shifts.parent_id={7}
    {8}and courses.campus_id in ({9})
