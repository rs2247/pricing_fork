CASE WHEN offers.university_id IN ({full_fixed_pos})
            ...
            ELSE
              ...
              ELSE
                ...
                ELSE
                  ...
                  THEN
                    CASE WHEN ({k} * (({real_discount}) * 1 + 0.5) * ({offered_balcao})) > CASE WHEN {k} > 1 THEN {min_7_plus} ELSE {min} END 
                    THEN ({k} * (({real_discount}) * 1 + 0.5) * ({offered_balcao})) 
                    ELSE CASE WHEN {k} > 1 THEN {min_7_plus} ELSE {min} END
                    END
                  ELSE
                    CASE WHEN ({offered_qb} * 0.5) > CASE WHEN {k} > 1 THEN {min_7_plus} ELSE {min} END 
                    THEN ({offered_qb} * 0.5) 
                    ELSE CASE WHEN {k} > 1 THEN {min_7_plus} ELSE {min} END
                    END
                  END    
